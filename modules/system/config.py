...
def first_setup(CONFIG, MOD, args=[]):
    ...
    if tools.yesno(MOD, "Do you want to setup the 'as_ext_host' paths too?"):
    ...
    if tools.yesno(MOD, "This project offers a YouTube downloader (which works for more than just YouTube). Do you want to set up the paths for it too?"):
        valid_ytdl_paths = []
        while True:
